# -*- codeing = utf-8 -*-
# @Time : 2022/4/17 10:28
# @Author : Ranyinze
# @File : FPI_Monthly_statical.py.py
# @Software: PyCharm
from Draw_statical_analysis import Drawfit
...
def GetMonthfog(filepath):
    ...
    for root, dirs, files in os.walk(filepath):
        ...
        for i in dirs:
            ...
            filepath_day.append(daily_filepath)
    # 获取该月份每一天的hdf文件名称
    filename_month = []
    for ii in filepath_day:
        for root1, dirs1, files1 in os.walk(ii):
            for iii in files1:
                if os.path.splitext(iii)[1]=='.HDF':
                    filename_month.append(os.path.join(ii,iii))
    # print(filename_month)
    # 循环读取这个月的所有hdf文件到一个矩阵，并将矩阵添加到列表中存储
    month_data = []
    for j in filename_month:
...
if __name__=="__main__":
    ...
    for ii in range(len(month111)):
        filepath = r'D:\future_study\FPI\DAILY\YYYYMM'
        outpath = r'D:\future_study\FPI\MONTH'
        filepath = filepath.replace('YYYY',year)
        filepath = filepath.replace('MM',month111[ii])

        # 读取一个月的数据矩阵到列表, 将一个月的hdf数据全部存储到列表中，30个array
        data_monthily = GetMonthfog(filepath)
        lines = 0
        for jj in range(len(data_monthily)):
            lines = len(data_monthily[jj]) + lines
        # 月合成的所有站点，其中有重复的站点
        month_data = np.empty((lines,5))
        # print(month_data.shape)
        curr_line = 0
        for jjj in range(len(data_monthily)):
            # print(len(data_monthily[jjj]))
            # print(curr_line+data_monthily[jjj].shape[0])
            month_data[curr_line: curr_line+data_monthily[jjj].shape[0],:] = data_monthily[jjj]
            curr_line = curr_line + len(data_monthily[jjj])
        # 对月合成站点 month_data 统计 出现的次数，最大值应该为当月的天数
        # 1. 取出经纬度
        lon_lat_arr = month_data[:,0:2]
        ...
        # 3. 赋值
        # 1-5列表示之前的属性值，第6列表示该站点在该月出现雾的天数，第7列表是频率 = 天数/当月天数
        Month_Fog = np.zeros((len(index), 7))
        for s in range(len(index)):
            Month_Fog[s, 0:5] = month_data[index[s], :]
            Month_Fog[s, 5:6] = counts[s]
        Month_Fog[:,-1] = Month_Fog[:,-2]/len(data_monthily)
        print(Month_Fog)
        ...
        print(Month_Fog.shape)
        outname1 = curr_month + '_Alt.png'
        outname2 = curr_month + '_Tmp.png'
        outpngname_alt = os.path.join(outpath, outname1)
        outpngname_tmp = os.path.join(outpath, outname2)
        m = os.path.splitext(outname)[0][-2:]
        Drawfit.altitude(Month_Fog[:, 2],Month_Fog[:, -1], int(m), outpngname_alt)
        Drawfit.temperature(Month_Fog[:, 4], Month_Fog[:, -1], int(m), outpngname_tmp)
        # 输出excel
